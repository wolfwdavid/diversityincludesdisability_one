...
<task type="auto">
  ...
  <acceptance_criteria>
    ...
    - `grep -c "href=\"/\", label: 'Home'" src/lib/data/nav.ts` shows navItems still has exactly 7 entries (statement NOT added to primary nav)
...
<task type="auto">
  ...
  <action>
    ...
    3. Extend `tests/deploy.smoke.spec.ts`: add a test `DEPLOY: /accessibility/ deep-link resolves` (goto('accessibility/'), status < 400, h1 matches /accessibility statement/i) and a test `404: branded fallback renders on deploy` reusing the existing unknown-path pattern (goto a random path, body contains /Diversity Includes Disability/i). Use RELATIVE goto (no leading slash) per the existing sub-path comment.
