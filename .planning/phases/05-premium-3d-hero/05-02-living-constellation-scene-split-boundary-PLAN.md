...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/lib/components/premium/SceneCanvas.svelte"
      provides: "the SOLE <Canvas renderMode='on-demand' dpr={[1,2]}> owner (second import hop)"
      contains: "dpr={[1, 2]}"
    - path: "src/lib/components/premium/scene/Scene.svelte"
      provides: "camera + lights + Orbs + Connections + useTask running-gate + parallax tilt + onReady first-frame + onDestroy disposal"
      contains: "renderer.forceContextLoss"
    - path: "src/lib/components/premium/scene/Orbs.svelte"
      provides: "InstancedMesh of 40 (desktop)/20 (mobile) periwinkle white-core orbs, drift+breathe, edge-banded seeds"
      contains: "InstancedMesh"
    - path: "src/lib/components/premium/scene/Connections.svelte"
      provides: "LineSegments proximity graph, per-frame BufferGeometry + setDrawRange"
...
Output: 7 premium components + the Home hero wiring; `test:no-three` flips GREEN; the premium canvas-mount + dispose specs go green.
</objective>
...
<task type="auto">
  <name>Task 1: Four-level boundary — PremiumHero gate + HeroScene + SceneCanvas</name>
  <read_first>
    ...
    - C:\Users\Mkaru\Documents\Hello_World\hugginface_profile\Websites\diversityincludesdisability_one\.planning\phases\05-premium-3d-hero\05-RESEARCH.md (§Lazy-Import Boundary — the four-level structure + why the SECOND import() hop and no scoped <style>)
  </read_first>
  ...
  <action>
    ...
    {/if}
    ```
    `HeroScene.svelte` (second-hop wrapper — NO scoped `<style>`; `.hero-scene` class comes from app.css):
    ```svelte
    <script lang="ts">
    ...
    ```
    `SceneCanvas.svelte` (SOLE `<Canvas>` owner — NO scoped `<style>`; dpr `[1, 2]` per D-13):
    ```svelte
    <script lang="ts">
    ...
    ```
    Rules that are HERO-02/04 failures if broken: only files under `premium/` may import three/@threlte; PremiumHero must stay three-free; HeroScene and SceneCanvas carry NO scoped `<style>`; the Canvas is reachable only via the two nested `import()` hops.
  </action>
  <verify>
  ...
  <done>Four-level boundary shell built: three-free gate reading the theme store + notLowPower, second-hop HeroScene (aria-hidden, no scoped style), SceneCanvas sole Canvas at dpr [1,2] on-demand; sceneReady/onReady/onContextLost plumbing complete.</done>
</task>

<task type="auto" tdd="true">
  <name>Task 2: The Living Constellation scene (Scene + Orbs + Connections + Lights)</name>
  <behavior>
  ...
  <files>src/lib/components/premium/scene/Scene.svelte, src/lib/components/premium/scene/Orbs.svelte, src/lib/components/premium/scene/Connections.svelte, src/lib/components/premium/scene/Lights.svelte</files>
  <action>
    Create four components under `src/lib/components/premium/scene/`.

    `Scene.svelte` — copy `_four`'s Scene disposal skeleton VERBATIM, then adapt children + add onReady + parallax:
      - `const { renderer, invalidate } = useThrelte(); const canvas = renderer.domElement;`
      - Running gate: `visible` (visibilitychange) AND `onscreen` (IntersectionObserver threshold 0 on canvas) → `running = () => visible && onscreen` (D-07 RAF pause).
      ...
      - `<T.PerspectiveCamera makeDefault position={[0,0,6]} fov={50} />`, `<Lights />`, and the group with `<Orbs {elapsed} count={COUNT} />` + `<Connections {elapsed} count={COUNT} />` where `const COUNT = matchMedia('(max-width: 640px)').matches ? 20 : 40;` (D-04/D-13).
    ...
    `Orbs.svelte` (D-01/D-02/D-04) — `InstancedMesh` of a small `SphereGeometry(0.05, 12, 12)`; `MeshStandardMaterial({ color: new Color('#7aa2ff'), emissive: new Color('#a9c2ff'), emissiveIntensity: 1.1 })` for periwinkle bodies with white-hot read; per-instance seeds `{ basePos, driftPhase, size, coreTint }`. Seed positions into LEFT/RIGHT bands (exclusion zone, D-11) keeping the central text column clear — e.g. `x = (±)(1.6 + rand*1.8)`, avoid |x| < 1.2. In an `$effect` on `elapsed`: slow sinusoidal drift + breathe scale `s = 1 + 0.06*Math.sin(elapsed*ω + phase)` (small ω ≈ 0.3–0.5 for the 20–30s contemplative cycle, D-05/D-08); `mesh.setMatrixAt(i, dummy.matrix)`; `mesh.instanceMatrix.needsUpdate = true`. White-hot core: high emissiveIntensity is acceptable; a two-layer opaque-core + additive-halo look is Claude's discretion. `onDestroy(() => { geo.dispose(); mat.dispose(); })`. Accept a `count` prop.

    `Connections.svelte` (D-03, proximity graph) — one `THREE.LineSegments` with a `BufferGeometry` pre-sized for max pairs `N*(N-1)/2 * 2` vertices (N=40 → ≤1560; negligible). Each frame (an `$effect` on `elapsed`, reading the same orb positions — share the seed/position logic with Orbs, e.g. export a positions helper or recompute the same drift): walk unique pairs; for pairs within a distance threshold (Claude's-discretion tunable, e.g. ~1.4 world units) write both endpoints into the position attribute and a per-vertex alpha/color falling off with distance; `geometry.setDrawRange(0, activeVertexCount)`; `position.needsUpdate = true` (+ color attr). Material: `LineBasicMaterial({ transparent: true, vertexColors: true, blending: AdditiveBlending })` in the periwinkle range, low base opacity. `onDestroy(() => { geometry.dispose(); material.dispose(); })`. Accept a `count` prop.
  ...
  <acceptance_criteria>
    - All four files exist under `src/lib/components/premium/scene/`
  ...
  <done>The periwinkle constellation renders: 40/20 instanced white-core orbs on edge bands, a proximity LineSegments graph, monochrome lights, contemplative drift+breathe, optional parallax tilt, RAF pause offscreen, first-frame onReady, and full imperative disposal. Boundary gate GREEN.</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    5. Drive the scene specs GREEN using the Windows-safe harness (Pitfall 6 — managed webServer hangs at teardown on this box):
       ...
       Target: the accessible-branch, premium-canvas-mount, bundle, and dispose specs pass. (The reduced-motion POSTER assertion and the accessible NO-poster-image assertion depend on the poster from Plan 05-03 — those two remain RED here and are 05-03's drive-green target; note that explicitly in the SUMMARY.)
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    - `e2e/premium-bundle.spec.ts` and `e2e/premium-dispose.spec.ts` pass (HERO-02 lazy-load + HERO-04 no-leak / context-loss)
  ...
  <done>The constellation is a full-bleed decorative background behind the untouched hero text, with a scrim + 85svh Premium sizing (Accessible untouched); three stays split (boundary GREEN); the bundle + dispose + canvas-mount specs are green.</done>
</task>
...
- `npx svelte-check` clean across all 7 premium components + +page.svelte.
- Premium canvas-mount, bundle-network, and dispose specs pass via the self-managed preview.
